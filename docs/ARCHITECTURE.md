...
网页通过 Worker 的 `GET /export` 获得完整业务数据，在浏览器加入 SHA-256 内容校验后下载 JSON。Worker 响应不含密码派生值、盐、Bearer token、设备标识摘要或登录失败记录。JSON 恢复只导入记录，习惯、设置、月度重点和会话只用于预览。

完整灾难恢复使用 D1 SQL。`ops/backup-repo/` 保存未来私有备份仓库的 Actions 模板：每日导出、独立恢复校验、age 公钥加密、密文 SHA-256、7 日/4 周/6 月保留策略和受 lease 保护的 `backups` 快照分支。模板不在主仓库的工作流目录，阶段 4 不会触发，也没有配置任何云端凭据。

详细恢复边界见 `docs/BACKUP_AND_RESTORE.md`。

阶段 4 不恢复图片、多人、邀请或分享。阶段 5 才创建私有备份仓库、复制并启用定时工作流、配置 Secrets/Variables、部署 Worker、填写正式 `VITE_API_BASE_URL`、启用 Pages、确认额度保护并决定是否加入 PWA。
